  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      {open && (
        ...
        >
          <p className="label" style={{ margin: "0 0 6px" }}>{t("Modo")}</p>
          <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
            <button className="btn" style={{ flex: 1, padding: "4px 6px", fontSize: 11 }} onClick={() => setTheme("dark")}>{t("escuro")}</button>
            <button className="btn" style={{ flex: 1, padding: "4px 6px", fontSize: 11 }} onClick={() => setTheme("light")}>{t("claro")}</button>
          </div>
          <p className="label" style={{ margin: "0 0 6px" }}>{t("Paleta")}</p>
