...
> ⚠️ **O que publica é o `athena publish`, não os arquivos.** Desde a fase 4, o site serve do Supabase. O Claude continua só escrevendo/apagando arquivos em `Resumos/`; quem leva a mudança pro banco é o passo `[2/2]` do `athena.bat`. Como o publish é **espelho**, apagar a página do disco é o que a remove do banco — não existe passo manual de "apagar do Supabase".
...
Gera questões de fixação sobre a aula, com gabarito comentado ao final. Salva em `Resumos/reviews/<CODIGO>-Nome/<AULA>-review.md` — pasta separada, para a pasta da matéria ter só aulas. Vai pro banco como qualquer página, com o mesmo slug e a mesma matéria de sempre.

Regras específicas do review:

- **Mesmo contrato do ingest**: escrever `FAIL` no `.ingest-status` ao começar, registrar no `log.md` (mesma linha canônica, marcando `review`) e só então sobrescrever com `OK`. Sem isso o `.bat` não publica e o review nunca aparece no site. Não há mais passo de espelho — o `athena publish` lê direto de `Resumos/`
...
🔒 **`Notes/` NUNCA é apagado pelo delete.** O delete remove o **conteúdo gerado e publicado**, não o material do usuário. As notas cruas em `Notes/subjects/` são trabalho do aluno (podem ser rascunho de aula ainda não publicada) e os PDFs em `Notes/INATEL/` são o arquivo de referência — ambos ficam intactos. Pra apagar nota crua, o usuário faz manualmente.
...
1. `Resumos/subjects/<CODIGO>-*/<AULA>.md` (a página gerada)
2. `athena-web/public/materials/<CODIGO>-*/<AULA>.<ext>` (a cópia do material, se houver)
3. `Resumos/reviews/<CODIGO>-*/<AULA>-review.md`, se existir review dessa aula
...
O `index.md` NÃO muda (a matéria continua existindo). `Notes/` não é tocado.

### Remover uma MATÉRIA inteira

1. `Resumos/subjects/<CODIGO>-*/` (o conteúdo gerado, incluindo o MOC)
...
`Notes/subjects/<CODIGO>-*/` e `Notes/INATEL/<CODIGO>-*/` continuam intactos.

### Regra geral

**Nenhum `[[wikilink]]` pode sobreviver ao arquivo que ele aponta.** Depois de qualquer remoção, varrer MOC e `index.md` atrás de links órfãos — eles viram nó fantasma no grafo do Obsidian e 404 no site. Wiki órfã (página publicada sem motivo) e link órfão são os bugs mais comuns deste sistema — não deixar sobras no que é **gerado**. O que está em `Notes/` é do usuário e permanece.
